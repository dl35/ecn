...
private myValidator(myform: FormControl ): any {
    const tarray = <FormArray> myform.get('idlic') ;
    const gtype = <FormGroup> myform.get('type') ;
    if (  !myform.get('idcompet').value &&  tarray.length === 0 ) {
       return {formError: true} ;
    } else {
    // tslint:disable-next-line:max-line-length
    if ( myform.get('idcompet').value  && gtype.get('ok').value === false && gtype.get('ko').value === false && gtype.get('at').value === false ) {
...
private sendMail() {
  ...
  const json =  this.mailForm.value ;
...
private setChange(value) {
  const tarray = <FormArray>this.mailForm.get('idlic') ;
  let remove = false;
